...
public class Player
{
    ...
    {
        Debug.Log(string.Format("{0} Begins",IsSelf?"Player":"AI"));
        if(!IsSelf) Discard();
    }
    ...
    {
		Debug.Log(string.Format("{0} DrawCards",IsSelf?"Player":"AI"));
    ...
    public void Discard()
    {
		Debug.Log(string.Format("{0} Discard",IsSelf?"Player":"AI"));
        //随机出牌
    ...
    {
		Debug.Log(string.Format("{0} OnDiscard",IsSelf?"Player":"AI"));
		DrawCards();
		EndTurn();
    }
    ...
    public void EndTurn()
    {
        Debug.Log(string.Format("{0} Ends", IsSelf ? "Player" : "AI"));
