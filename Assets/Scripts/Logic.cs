...
public class GameData
{
    ...
    public Deck GetRandomAvailableSide(bool isSelf = false)
    {
        ...
        else  return OppoDecks.First (d => d.Count < d.MaxCount);
        return null;
    }
...
public delegate DeckView GetDeckView();
...
public class Deck : IList<Card>, IComparable<Deck>
{
    ...
    {
        StringBuilder sb = new StringBuilder("[" + _getDeckView().name + "] {");
    ...
    public void RemoveCard(Card card)
    {
        ...
        _list.Remove(card);
        Debug.Log(string.Format("<color=red>[{0}.RemoveCard] {1}</color>", _getDeckView().name, card));
    ...
    public void AddCard(Card card)
    {
        ...
        Debug.Log(string.Format("<color=green>[{0}.AddCard] {1}</color>", _getDeckView().name, card));
        card.AddToDeck(_getDeckView());
        card.RegisterRemoveFromDeckHandler(RemoveCard);
        _list.Add(card);
    ...
    {
        return _list[UnityEngine.Random.Range(0, this.Count)];
    }
    ...
    public void SortCard()
    {
        ...
        foreach (var card in _list)
        {
            card.SetOrder(_list.IndexOf(card));
        }
    }
    ...
    /// <summary>
    /// 注册用于获取Deck所对应的DeckView, 调用DeckView.GetDeckView
    /// </summary>
    /// <param name="getDeckView">对应的DeckView的GetDeckView委托</param>
    public void RegisterGetTransform(GetDeckView getDeckView)
    {
        _getDeckView = getDeckView;
    }
    private GetDeckView _getDeckView;
    #endregion
}
...
public delegate void AddToDeckView(DeckView view);
public class Card:IComparable<Card>,IEquatable<Card>
{
    ...
    private AddToDeckView onAdd;
    private SetOrder setOrder;
    public void RegisterRemoveFromDeckHandler(RemoveFromDeck onRemoveHandler)
    {
        onRemove = onRemoveHandler;
    }
    public void RegisterAddToDeckViewHandler(AddToDeckView onAddHandler)
    {
        onAdd = onAddHandler;
    }
    public void RegisterSetOrderHandler(SetOrder onSetOrder)
    {
        setOrder = onSetOrder;
    }
    ...
    public void AddToDeck(DeckView view)
    {
        onAdd(view);
		if (onRemove != null) onRemove(this);//调用Deck.Remove,从前一个牌组中移除卡牌
    }
